package com.m7.abs.admin.core.exception;
...
public class GlobalExceptionHandler {
    ...
    public ModelAndView defaultErrorHandler(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        if (ex instanceof AccessDeniedException) {
            HttpUtil.sendJson(response, BaseResponse.fail(ErrorCodeConstant.CommonErrorCode.PERMISSION_EXCEPTION.getCode(), "权限不够,不允许访问"));
            return null;
        }

        HttpUtil.sendJson(response, BaseResponse.fail(ErrorCodeConstant.CommonErrorCode.FAILED.getCode(), "System error."));
        return null;
    }

    @ExceptionHandler(value = AccessDeniedException.class)
    public ModelAndView accessDeniedHandler(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        HttpUtil.sendJson(response, BaseResponse.fail(ErrorCodeConstant.CommonErrorCode.PERMISSION_EXCEPTION.getCode(), ErrorCodeConstant.CommonErrorCode.PERMISSION_EXCEPTION.getErrMsg()));
        return null;
    }
