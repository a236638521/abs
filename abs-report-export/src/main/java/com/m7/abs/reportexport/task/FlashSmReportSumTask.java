...
@Slf4j
@Component
public class FlashSmReportSumTask {
    @Autowired
    ...
    public void flashSmReportSumTask() {
        String traceId = MyStringUtils.randomUUID();
        MDC.put(CommonSessionKeys.REQ_ID_KEY, traceId);
        log.info("Start flash sm report sum task.");
        flashSmReportSum(traceId, RedisKeys.LOCK_FLASH_SM_REPORT_SUM_TASK, FlashSmReportFields.CURRENT_NEED_REPORT_ACCOUNT, new Date());
    }
    ...
    public void flashSmReportDaySumTask() {
        String traceId = MyStringUtils.randomUUID();
        MDC.put(CommonSessionKeys.REQ_ID_KEY, traceId);
        log.info("Start flash sm day report sum task.");
        flashSmReportSum(traceId, RedisKeys.LOCK_FLASH_SM_REPORT_DAY_SUM_TASK, FlashSmReportFields.NEXT_DAY_NEED_REPORT_ACCOUNT, DateUtil.addDate(new Date(), Calendar.DATE, -1));
        flashSmCdrReportService.clearReportRedissonCache();
    }
     ...
     * @param date
     */
    private void flashSmReportSum(String traceId, String lockKey, String accountKey, Date date) {
        try {
            boolean lock = lockUtils.lock(RedisKeys.LOCK_FLASH_SM_REPORT, traceId);
            if (!lock) {
                log.info("not get lock, ignore it");
                return;
            }

            RSetCache<String> accountSet = redissonClient.getSetCache(RedisKeyConstant.FlashSms.getReportKeyFormatNoAccount(accountKey));
            if (accountSet != null && accountSet.size() > 0) {
                Set<String> accounts = accountSet.readAll();
                accounts.stream().forEach(account -> {
                    flashSmCdrReportService.sumCurrentFlashSmCdrReport(traceId, account, date);
                });
            }
            /**
             * 休眠60秒,60秒锁安全期
            ...
            lockUtils.unLock(RedisKeys.LOCK_FLASH_SM_REPORT, traceId);
        }
    }
