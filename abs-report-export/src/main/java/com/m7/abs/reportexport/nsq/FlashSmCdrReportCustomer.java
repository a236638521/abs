...
@Slf4j
@Component
public class FlashSmCdrReportCustomer {
    @Autowired
    protected JsonComponent jsonComponent;
    @Autowired
    private IFlashSmCdrReportService flashSmCdrReportService;

    @NsqListener(topic = "#{@absReportProperties.api.report.flashSmCdrReportTopic}",
            channel = "#{@absReportProperties.api.report.flashSmCdrReportChannel}")
    private void subscribeFlashSmCdrMsg(Message message) {
        try {
            byte[] data = message.getData();
            NsqMsgDTO<FlashSmReportDto> msg = null;
            try {
                TypeReference<NsqMsgDTO<FlashSmReportDto>> typeReference = new TypeReference<NsqMsgDTO<FlashSmReportDto>>() {
                };
            ...
            if (msg == null) {
                log.warn("无法解析闪信话单数据,消息为空", new String(data));
                return;
            }
            MDC.put(CommonSessionKeys.REQ_ID_KEY, msg.getTraceId());
            log.info("接收到闪信话单,NSQ消息:{}", FastJsonUtils.toJSONString(msg));
            flashSmCdrReportService.handleFlashSmCdrMsg(msg);
        } catch (Exception e) {
